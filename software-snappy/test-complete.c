...
#ifdef DO_PRINT
    printf("Took %" PRIu64 " cycles, produced %" PRIu64 " compressed bytes. uncomp size: %" PRIu64 " for benchmark: %s, with histsram: %" PRIu64 ", with log2HTEntries: %" PRIu64 "\n", t2 - t1, compressed_size, benchmark_uncompressed_data_len, bench_name, hist_size, hash_table_entries_log2);
#endif

    uint64_t t3 = rdcycle();
    bool uncompress_success = SnappyAccelRawUncompress(write_region, compressed_size, write_region_decomp);
    uint64_t t4 = rdcycle();

    //printf("Start cycle: %" PRIu64 "\n", t1);
    //printf("End cycle: %" PRIu64 "\n", t2);
#ifdef DO_PRINT
    printf("Took %" PRIu64 " cycles, produced %" PRIu64 " uncompressed bytes. comp size: %" PRIu64 " for benchmark: %s, with histsram: %" PRIu64 ", with log2HTEntries: %" PRIu64 "\n", t4 - t3, benchmark_uncompressed_data_len, compressed_size, bench_name, hist_size, hash_table_entries_log2);
#endif
...
//        128 << 10,
    ...
    unsigned int num_hash_table_sizes = 2;
...
//        10,
    ...
    unsigned char * result_area = SnappyCompressAccelSetup(total_benchmarks_uncompressed_size * 2, hist_sizes[0]);
    ...
    for (unsigned int k = 0; k < num_hash_table_sizes; k++) {
        ...
        SnappyCompressSetDynamicHashTableSizeLog2(hash_table_sizes_log2[k]);
#ifdef DO_PRINT
