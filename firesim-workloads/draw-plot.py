...
parser.add_argument("--bm-dir", type=str, default="../software/benchmarks/HyperCompressBench/extracted_benchmarks", help="HyperCompressBench extracted_benchmarks")
...
figsize_callsize_dist = (args.figsize_x//2, args.figsize_x//2)
fontsize_callsize_dist = fontsize * 1.2
labelsize_callsize_dist = labelsize / 1.0
...
def draw_call_size_distribution_plot(algo_name, comp_or_decomp):
  base_dir = args.bm_dir
  dir_name = f"{algo_name}-{comp_or_decomp}"
  dir_path = os.path.join(base_dir, dir_name)

  bash(f"ls -al {dir_path} > tmp")
  ...
  for line in lines:
    words = line.split()
    if len(words) >= 9  and ("cl" in words[8]) and ("comp" not in words[8]):
      try:
        filesize = int(words[4])
        bin_idx = find_log2_bin(filesize)
        cur_bytes = file_size_bytes[bin_idx]
        file_size_bytes[bin_idx] = cur_bytes + filesize
        max_bin_idx = max(max_bin_idx, bin_idx)
      except:
        print(words)

  total_bytes = sum(file_size_bytes.values())
  file_size_cumul_bytes = dict()
  cumul_bytes = 0
  for (k, v) in file_size_bytes.items():
    if k > max_bin_idx:
      file_size_cumul_bytes[k] = 0
      continue

    cumul_bytes += v
    file_size_cumul_bytes[k] = (cumul_bytes / total_bytes) * 100


  fig = plt.figure(figsize=figsize_callsize_dist, dpi=dpi, clear=True)
  ax = fig.add_subplot(1, 1, 1)

  if comp_or_decomp == "COMPRESS":
    reduce_comp_or_decomp = "COMP"
  else:
    reduce_comp_or_decomp = "DECOMP"

  ax.set_xlabel(f"{algo_name} {reduce_comp_or_decomp} call\n size, ceil(lg2(b))", fontsize=fontsize_callsize_dist)
  ax.set_ylabel("% of uncomp Bytes", fontsize=fontsize_callsize_dist)

  ax.tick_params(axis="x", which="major", direction="inout", labelsize=labelsize_callsize_dist)
  ax.tick_params(axis="y", which="major", direction="inout", labelsize=labelsize_callsize_dist)

  ax.bar(list(range(min_bin_size, max_bin_size)), list(file_size_cumul_bytes.values()))

  ax.grid(axis="both", linewidth=gridlinewidth)
  ax.yaxis.grid(visible=True)
  ax.set_axisbelow(True)

  ax.set_yticks(ticks=[0, 50, 100])
  ax.set_xticks(ticks=[15, 20, 25])

  plt.tight_layout()
  plt.savefig(os.path.join(args.output_dir, f"{algo_name}-{comp_or_decomp[0]}-callsizes.pdf"), format="pdf")

def draw_hyper_compress_bench_distribution_plot():
  draw_call_size_distribution_plot("ZSTD", "COMPRESS")
  draw_call_size_distribution_plot("ZSTD", "DECOMPRESS")
  draw_call_size_distribution_plot("Snappy", "COMPRESS")
  draw_call_size_distribution_plot("Snappy", "DECOMPRESS")


def main():
  draw_compression_plot()
  draw_decompression_plot()
  draw_hyper_compress_bench_distribution_plot()
