...
class SnappyCompressorMemwriter()(implicit p: Parameters) extends Module
  with MemoryOpConstants {

  val io = IO(new Bundle {
    val memwrites_in = Flipped(Decoupled(new CompressWriterBundle))
    val l2io = new L2MemHelperBundle
    val compress_dest_info = Flipped(Decoupled(new SnappyCompressDestInfo))
  ...
  val compress_dest_info_Q = Module(new Queue(new SnappyCompressDestInfo, 4))
  ...
  when (compress_dest_printhelp) {
    CompressAccelLogger.logInfo("[config-memwriter] got dest info op: 0x%x, boolptr 0x%x\n",
      ...
      compress_dest_info_Q.io.deq.bits.boolptr)
  }

  val buf_lens_Q = Module(new Queue(UInt(64.W), 10))
  when (buf_lens_Q.io.enq.fire) {
    CompressAccelLogger.logInfo("[memwriter-snappycomp] enqueued buf len: %d\n", buf_lens_Q.io.enq.bits)
  }
  ...
  when (incoming_writes_Q.io.deq.fire) {
    CompressAccelLogger.logInfo("[memwriter-snappycomp] dat: 0x%x, bytes: 0x%x, EOM: %d\n",
  ...
  when (mem_write_fire.fire) {
    ...
    backend_bytes_written := backend_bytes_written + bytes_to_write
    CompressAccelLogger.logInfo("[memwriter-snappycomp] writefire: addr: 0x%x, data 0x%x, size %d\n",
  ...
  val final_output_len = buf_lens_Q.io.deq.bits
  ...
  io.l2io.req.bits.addr := Mux(write_ptr_override, compress_dest_info_Q.io.deq.bits.boolptr, backend_next_write_addr)
  ...
  when (bool_ptr_write_fire.fire) {
    ...
    CompressAccelLogger.logInfo("[memwriter-snappycomp] write resultlen addr: 0x%x, write final output len 0x%x\n", compress_dest_info_Q.io.deq.bits.boolptr, final_output_len)
  }

  when (count_valids =/= 0.U) {
    CompressAccelLogger.logInfo("[memwriter-snappycomp] read_start_index %d, backend_bytes_written %d, count_valids %d, ptr_align_max_bytes_writeable %d, bytes_to_write %d, bytes_to_write_log2 %d\n",
      read_start_index,
