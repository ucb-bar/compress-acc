...
class HufCompressorCommandRouterIO()(implicit val p: Parameters) 
  extends Bundle {
  ...
  val cnt_info = Decoupled(new StreamInfo)
...
class HufCompressorCommandRouter(val cmd_que_depth: Int)(implicit p: Parameters) 
  extends Module {

  val io = IO(new HufCompressorCommandRouterIO)
  ...
  val cnt_info_queue = Module(new Queue(new StreamInfo, cmd_que_depth))
  val src_info_fire = DecoupledHelper(io.rocc_in.valid,
                                      src_info_queue.io.enq.ready,
                                      cnt_info_queue.io.enq.ready,
  ...
  cnt_info_queue.io.enq.bits.ip := cur_rs1
  cnt_info_queue.io.enq.bits.isize := cur_rs2
// cnt_info_queue.io.enq.bits.isize := (cur_rs2 >> 3.U) // Sample from the first 1/8 part of the stream
  cnt_info_queue.io.enq.valid := src_info_fire.fire(cnt_info_queue.io.enq.ready)
  io.cnt_info <> cnt_info_queue.io.deq

  when (io.cnt_info.fire) {
    CompressAccelLogger.logInfo("CommandRouter, io.cnt_info.fire!\n")
    CompressAccelLogger.logInfo("CommandRouter, io.cnt_info.ip: 0x%x\n", io.cnt_info.bits.ip)
    CompressAccelLogger.logInfo("CommandRouter, io.cnt_info.isize: 0x%x\n", io.cnt_info.bits.isize)
  }
