...
class ZstdDecompressor(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [cmpflag_writer]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := l2_cmpflag_writer.masterNode
  } else {
    println(s"latency injection OFF for [cmpflag_writer] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [fhdr_reader]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := l2_fhdr_reader.masterNode
  } else {
    println(s"latency injection OFF for [fhdr_reader] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
    tlNode := l2_fhdr_reader.masterNode
  }
  // For block header
  val l2_bhdr_reader = LazyModule(new L2MemHelper("[bhdr_reader]", numOutstandingReqs=4))
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [bhdr_reader]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := l2_bhdr_reader.masterNode
  } else {
    println(s"latency injection OFF for [bhdr_reader] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [huf_lit_reader]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := l2_huf_literal_reader.masterNode
  } else {
    println(s"latency injection OFF for [huf_lit_reader] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [huf_hdr_reader]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := l2_huf_header_reader.masterNode
  } else {
    println(s"latency injection OFF for [huf_hdr_reader] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    if (p(CompressAccelFarAccelLocalCache)) {
      println(s"latency injection OFF for [huf_lit_writer] due to CompressAccelFarAccelLocalCache: ${p(CompressAccelFarAccelLocalCache)}")
      tlNode := l2_huf_literal_writer.masterNode
    } else {
      println(s"latency injection ON for [huf_lit_writer]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
      tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := l2_huf_literal_writer.masterNode
    }
  } else {
    println(s"latency injection OFF for [huf_lit_writer] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
    tlNode := l2_huf_literal_writer.masterNode
  }
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [mem_decomp_ireader_dtbuilder]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := mem_decomp_ireader_dtbuilder.masterNode
  } else {
    println(s"latency injection OFF for [mem_decomp_ireader_dtbuilder] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [mem_decomp_ireader_dtreader]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := mem_decomp_ireader_dtreader.masterNode
  } else {
    println(s"latency injection OFF for [mem_decomp_ireader_dtreader] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    if (p(CompressAccelFarAccelLocalCache)) {
      println(s"latency injection OFF for [m_decomp_readbackref] due to CompressAccelFarAccelLocalCache: ${p(CompressAccelFarAccelLocalCache)}")
      tlNode := mem_decomp_ireader_histlookup.masterNode
    } else {
      println(s"latency injection ON for [m_decomp_readbackref]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
      tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := mem_decomp_ireader_histlookup.masterNode
    }
  } else {
    println(s"latency injection OFF for [m_decomp_readbackref] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
    tlNode := mem_decomp_ireader_histlookup.masterNode
  }
  ...
  if (p(CompressAccelLatencyInjectEnable)) {
    if (p(CompressAccelFarAccelLocalCache)) {
      println(s"latency injection OFF for [mem_decomp_ireader_seqexec] due to CompressAccelFarAccelLocalCache: ${p(CompressAccelFarAccelLocalCache)}")
      tlNode := mem_decomp_ireader_seqexec.masterNode
    } else {
      println(s"latency injection ON for [mem_decomp_ireader_seqexec]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
      tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := mem_decomp_ireader_seqexec.masterNode
    }
  } else {
    println(s"latency injection OFF for [mem_decomp_ireader_seqexec] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
    tlNode := mem_decomp_ireader_seqexec.masterNode
  }
	...
	if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [m_decomp_writer_seqexec]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := mem_decomp_writer_seqexec.masterNode
  } else {
    println(s"latency injection OFF for [m_decomp_writer_seqexec] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
  ...
  val mem_decomp_ireader_rawrle = LazyModule(new L2MemHelper("[mem_decomp_ireader_rawrle]", numOutstandingReqs=32))
	if (p(CompressAccelLatencyInjectEnable)) {
    println(s"latency injection ON for [mem_decomp_ireader_rawrle]: adding ${p(CompressAccelLatencyInjectCycles)} cycles")
    tlNode := TLBuffer.chainNode(p(CompressAccelLatencyInjectCycles)) := mem_decomp_ireader_rawrle.masterNode
  } else {
    println(s"latency injection OFF for [mem_decomp_ireader_rawrle] due to CompressAccelLatencyInjectEnable: ${p(CompressAccelLatencyInjectEnable)}")
