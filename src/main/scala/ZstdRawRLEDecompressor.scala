...
class ZstdRawRLEDecompressor(l2bw: Int)(implicit p: Parameters) extends Module{
  ...
  // val l2bwdiv8_uint = Wire(UInt(21.W)) //avoid verilator error
  // l2bwdiv8_uint := (l2bw/8).U //avoid verilator error
  // val write_requests_to_put = (io.block_size-1.U)/l2bwdiv8_uint + 1.U //avoid verilator error
