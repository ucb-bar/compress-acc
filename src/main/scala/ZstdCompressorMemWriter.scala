...
class ZstdCompressorMemWriter(val circularQueDepth: Int = 16, val writeCmpFlag: Boolean = true, val printinfo: String = "zcmw")
  ...
  val io = IO(new Bundle {
    ...
    val dest_info = Flipped((Decoupled(new DstWithValInfo)))

    val bufs_completed = Output(UInt(64.W))
    val no_writes_inflight = Output(Bool())
  })

  val incoming_writes_Q = Module(new Queue(new WriterBundle, queDepth))

  incoming_writes_Q.io.enq <> io.memwrites_in

  val dest_info_Q = Module(new Queue(new DstWithValInfo, queDepth))
  dest_info_Q.io.enq <> io.dest_info

  val decompress_dest_last_fire = RegNext(dest_info_Q.io.deq.fire)
  val decompress_dest_last_valid = RegNext(dest_info_Q.io.deq.valid)
  val decompress_dest_printhelp = dest_info_Q.io.deq.valid && (decompress_dest_last_fire || (!decompress_dest_last_valid))
  ...
  val NUM_QUEUES = 32
  val QUEUE_DEPTHS = circularQueDepth
...
// when (input_fire_allqueues.fire && !end_of_buf) {
  ...
  io.l2io.req.bits.data := Mux(write_ptr_override, dest_info_Q.io.deq.bits.cmpval, remapped_write_data)
