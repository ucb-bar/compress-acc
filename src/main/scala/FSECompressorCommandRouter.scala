...
class FSECompressorCommandRouterIO()(implicit val p: Parameters) 
  extends Bundle {
  ...
  val nbseq_info = Decoupled(UInt(64.W))
}

class FSECompressorCommandRouter(val cmd_que_depth: Int)(implicit p: Parameters) 
  extends Module {

  val io = IO(new FSECompressorCommandRouterIO)
  ...
  io.nbseq_info <> nbseq_info_queue.io.deq
