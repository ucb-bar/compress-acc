...
class CompressWriterBundle extends Bundle {
  val data = UInt(256.W)
  val validbytes = UInt(6.W)
  // both of these can be used to implicitly convey to the memwriter
  // the end of a bunch of literal pieces (and it will automatically know
  // the lengths by tracking and putting in an internal queue)
...
class ZstdMatchFinderMemwriter(printInfo: String, writeCmpFlag: Boolean = true)(implicit p: Parameters) extends Module
  with MemoryOpConstants {

  val io = IO(new Bundle {
    val memwrites_in = Flipped(Decoupled(new CompressWriterBundle))
    val l2io = new L2MemHelperBundle
    val compress_dest_info = Flipped(Decoupled(new DstInfo))

    val bufs_completed = Output(UInt(64.W))
    val no_writes_inflight = Output(Bool())
    val written_bytes = Decoupled(UInt(64.W))
  ...
  val compress_dest_info_Q = Module(new Queue(new DstInfo, 4))
  compress_dest_info_Q.io.enq <> io.compress_dest_info

  val compress_dest_last_fire = RegNext(compress_dest_info_Q.io.deq.fire)
  val compress_dest_last_valid = RegNext(compress_dest_info_Q.io.deq.valid)
  val compress_dest_printhelp = compress_dest_info_Q.io.deq.valid && (compress_dest_last_fire || (!compress_dest_last_valid))

  when (compress_dest_printhelp) {
    CompressAccelLogger.logInfo("[config-memwriter] got dest info op: 0x%x, cmpflag 0x%x\n",
      compress_dest_info_Q.io.deq.bits.op,
      compress_dest_info_Q.io.deq.bits.cmpflag)
  }
  ...
  for ( queueno <- 0 until NUM_QUEUES ) {
    ...
    for (j <- 0 until NUM_QUEUES) {
      when (j.U === idx) {
        mem_resp_queues(j).enq.bits := (incoming_writes_Q.io.deq.bits.data >> (queueno.U << 3))(7, 0)
      }
    }
  }
  ...
  val account_for_no_dummy_buf_lens_Q = (!end_of_buf) || (end_of_buf && no_dummy_buf_lens_Q.io.enq.ready)

  val input_fire_allqueues = DecoupledHelper(
    incoming_writes_Q.io.deq.valid,
    all_queues_ready,
    account_for_buf_lens_Q,
    account_for_no_dummy_buf_lens_Q
  )
  ...
  no_dummy_buf_lens_Q.io.enq.valid := input_fire_allqueues.fire(account_for_no_dummy_buf_lens_Q) && end_of_buf
  no_dummy_buf_lens_Q.io.enq.bits := buf_len_tracker +& Mux(incoming_writes_Q.io.deq.bits.is_dummy, 0.U, incoming_writes_Q.io.deq.bits.validbytes)

  when (buf_lens_Q.io.enq.fire) {
    CompressAccelLogger.logInfo("[" + printInfo + "]" + "buf_lens_Q.io.enq.fire\n")
    CompressAccelLogger.logInfo("[" + printInfo + "]" + "buf_lens_Q.io.enq.bits: %d\n",
      buf_lens_Q.io.enq.bits)
  }

  when (no_dummy_buf_lens_Q.io.enq.fire) {
    CompressAccelLogger.logInfo("[" + printInfo + "]" + "no_dummy_buf_lens_Q.io.enq.fire\n")
    CompressAccelLogger.logInfo("[" + printInfo + "]" + "no_dummy_buf_lens_Q.io.enq.bits: %d\n",
      no_dummy_buf_lens_Q.io.enq.bits)
  }
  ...
  val backend_next_write_addr = compress_dest_info_Q.io.deq.bits.op + backend_bytes_written
  ...
  // TODO: when choosing bytes_to_write, account for amount left in this buf
  ...
  val mem_write_fire = DecoupledHelper(
    ...
    enough_data,
    !write_ptr_override,
    compress_dest_info_Q.io.deq.valid
  )

  val written_bytes_q = Module(new Queue(UInt(64.W), 10))
  io.written_bytes <> written_bytes_q.io.deq

  val bool_ptr_write_fire = DecoupledHelper(
    io.l2io.req.ready,
    buf_lens_Q.io.deq.valid,
    buf_lens_Q.io.deq.bits === backend_bytes_written,
    no_dummy_buf_lens_Q.io.deq.valid,
    compress_dest_info_Q.io.deq.valid,
    written_bytes_q.io.enq.ready
  )
  ...
  val final_output_len = no_dummy_buf_lens_Q.io.deq.bits
  io.l2io.req.bits.size := Mux(write_ptr_override, 2.U, bytes_to_write_log2)
  io.l2io.req.bits.addr := Mux(write_ptr_override, compress_dest_info_Q.io.deq.bits.cmpflag, backend_next_write_addr)
  io.l2io.req.bits.data := Mux(write_ptr_override, final_output_len, remapped_write_data)
  io.l2io.req.bits.cmd := M_XWR
  if (writeCmpFlag) {
    io.l2io.req.valid := mem_write_fire.fire(io.l2io.req.ready) || bool_ptr_write_fire.fire(io.l2io.req.ready, io.force_write)
  } else {
  ...
  no_dummy_buf_lens_Q.io.deq.ready := bool_ptr_write_fire.fire(no_dummy_buf_lens_Q.io.deq.valid)
  compress_dest_info_Q.io.deq.ready := bool_ptr_write_fire.fire(compress_dest_info_Q.io.deq.valid)

  written_bytes_q.io.enq.valid := bool_ptr_write_fire.fire(written_bytes_q.io.enq.ready)
  written_bytes_q.io.enq.bits := final_output_len
  ...
  when (bool_ptr_write_fire.fire) {
    ...
    CompressAccelLogger.logInfo("[" + printInfo + "] write resultlen addr: 0x%x, write final output len 0x%x\n", compress_dest_info_Q.io.deq.bits.cmpflag, final_output_len)
  }

  when (count_valids =/= 0.U) {
    CompressAccelLogger.logInfo("[" + printInfo + "] read_start_index %d, backend_bytes_written %d, count_valids %d, ptr_align_max_bytes_writeable %d, bytes_to_write %d, bytes_to_write_log2 %d\n",
      read_start_index,
