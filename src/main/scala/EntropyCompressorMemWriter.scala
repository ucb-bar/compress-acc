...
class EntropyCompressorMemwriter(val writeCmpFlag: Boolean = false, val printInfo: String = "entropy-memwriter")(implicit p: Parameters) extends Module
  ...
  when (decompress_dest_printhelp) {
    CompressAccelLogger.logInfo("[" + printInfo + "] got dest info op: 0x%x, cmpflag 0x%x\n",
  ...
  io.l2io.req.bits.size := Mux(write_ptr_override, 2.U, bytes_to_write_log2) // FIXME : correct literal header formatting
  ...
  when (bool_ptr_write_fire.fire) {
    ...
    CompressAccelLogger.logInfo("[" + printInfo + "] write cmpflag addr: 0x%x, final_bytes_written: %d\n", decompress_dest_info_Q.io.deq.bits.cmpflag, final_bytes_written)
  }

  when (count_valids =/= 0.U) {
    CompressAccelLogger.logInfo("[" + printInfo + "] write_start_index %d, backend_bytes_written %d, count_valids %d, ptr_align_max_bytes_writeable %d, bytes_to_write %d, bytes_to_write_log2 %d, l2io.req.ready: %d, enough_data: %d, write_ptr_override: %d, decompress_dest_info_Q.io.deq.valid: %d\n",
      read_start_index,
