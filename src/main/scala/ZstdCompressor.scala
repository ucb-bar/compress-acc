...
class ZstdCompressor(opcodes: OpcodeSet)(implicit p: Parameters) 
  ...
  val l2_fhdr_writer = LazyModule(new L2MemHelper("[fhdr_writer]",
                                                  numOutstandingReqs=4, printWriteBytes=true))
  tlNode := l2_fhdr_writer.masterNode

  val l2_bhdr_writer = LazyModule(new L2MemHelper("[bhdr_writer]",
                                                  numOutstandingReqs=4, printWriteBytes=true))
...
class ZstdCompressorImp(outer: ZstdCompressor)(implicit p: Parameters) 
  ...
  matchfinder.io.MAX_OFFSET_ALLOWED := controller.io.shared_control.mf_maxoffset
  matchfinder.io.RUNTIME_HT_NUM_ENTRIES_LOG2 := controller.io.shared_control.mf_runtime_ht_num_entries_log2
