...
class HufCompressorMemwriter(val writeCmpFlag: Boolean = false)(implicit p: Parameters) extends Module
  ...
  val decompress_dest_info_Q = Module(new Queue(new DstInfo, 4))
  ...
  when (buf_lens_Q.io.enq.fire) {
    CompressAccelLogger.logInfo("[memwriter-hdrwriter] enqueued buf len: %d\n", buf_lens_Q.io.enq.bits)
  }
  ...
  when (incoming_writes_Q.io.deq.fire) {
    CompressAccelLogger.logInfo("[memwriter-hdrwriter] dat: 0x%x, bytes: 0x%x, EOM: %d\n",
  ...
  for ( queueno <- 0 until NUM_QUEUES ) {
    mem_resp_queues((write_start_index +& UInt(queueno)) % UInt(NUM_QUEUES)).enq.bits := (incoming_writes_Q.io.deq.bits.data >> (queueno.U << 3))(7, 0)
  }
  ...
  when (mem_write_fire.fire) {
    ...
    CompressAccelLogger.logInfo("[memwriter-hdrwriter] writefire: addr: 0x%x, data 0x%x, size %d\n",
...
// val final_bytes_written = buf_lens_Q.io.deq.bits
  val final_bytes_written = io.custom_write_value.bits
  io.custom_write_value.ready := bool_ptr_write_fire.fire(io.custom_write_value.valid)
  ...
  io.l2io.req.bits.data := Mux(write_ptr_override, final_bytes_written, remapped_write_data)
  ...
  when (bool_ptr_write_fire.fire) {
    ...
    CompressAccelLogger.logInfo("[memwriter-hdrwriter] write cmpflag addr: 0x%x, final_bytes_written: %d\n", decompress_dest_info_Q.io.deq.bits.cmpflag, final_bytes_written)
  }

  when (count_valids =/= 0.U) {
    CompressAccelLogger.logInfo("[memwriter-hdrwriter] write_start_index %d, backend_bytes_written %d, count_valids %d, ptr_align_max_bytes_writeable %d, bytes_to_write %d, bytes_to_write_log2 %d\n",
      read_start_index,
