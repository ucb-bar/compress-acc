...
class BufferManager(l2bw: Int)(implicit p: Parameters) extends Module{
    ...
    */
    ...
    when(io.flush){
        ...
        when(buffer_ptr === 0.U){
            io.most_recent_stream := most_recent_buffer
        }.otherwise{
            val bits_recent = l2bw.U-buffer_ptr
            val bits_reservoir = buffer_ptr
            bitmask := (1.U << bits_recent) - 1.U
            bitmask_reservoir := (1.U << bits_reservoir) - 1.U
            val data_from_recent = most_recent_buffer & bitmask
            val data_from_reservoir = reservoir_buffer & bitmask_reservoir
            io.most_recent_stream := (data_from_reservoir << bits_recent) + data_from_recent
        }
