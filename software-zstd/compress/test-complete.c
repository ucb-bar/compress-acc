...
bool run_benchmark(char * benchmark_compressed_data, size_t benchmark_compressed_data_len,
    ...
    char * bench_name, unsigned int benchno, uint64_t hist_size, char * write_region_decomp, uint64_t hash_table_entries_log2, latency_info_t latency_info,
  ...
  printf("Took %" PRIu64 " cycles produced %" PRIu64 " compressed bytes uncompsize %" PRIu64 " for benchmark %s with histsram %" PRIu64 " with log2HTEntries: %" PRIu64 " with latencyInjection: %" PRIu64 " hasCache: %d\n", 
        t2 - t1, compressed_size, benchmark_uncompressed_data_len, bench_name, hist_size, hash_table_entries_log2, latency_info.cycles, latency_info.has_cache);
...
/* printf("Checking compressed output correctness:\n"); */
  ...
  for (size_t i = 0; i < bench_num_words; i++) {
    if (benchmark_uncompressed_data_by8[i] != result_area_by8[i]) {
      ...
      if (first_fail) {
        ...
        printf("FAIL INDEX %" PRIu64 " bench_num_words %" PRIu64 "\n", i, bench_num_words);
        printf("Expected %" PRIu64 " Got %" PRIu64 "\n", benchmark_uncompressed_data_by8[i], result_area_by8[i]);
        first_fail = false;
        break;
      }
  ...
  for (size_t i = tail_start; i < benchmark_uncompressed_data_len; i++) {
    ...
    if (benchmark_uncompressed_data[i] != write_region_decomp[i]) {
      ...
      if (first_fail) {
        printf("FAIL ON BENCHMARK! N: %d, name: %s, with histsram: %" PRIu64 ", log2HTEntries: %" PRIu64 "\n", benchno, bench_name, hist_size, hash_table_entries_log2);
        printf("FAIL INDEX %" PRIu64 " bench_uncompressed_data_len%" PRIu64 "\n", i, benchmark_uncompressed_data_len);
        printf("Expected %" PRIu64 " Got %" PRIu64 "\n", benchmark_compressed_data[i], write_region_decomp[i]);
        first_fail = false;
      }
#ifdef DO_PRINT
...
int main() {
  ...
  printf("Shard Uncompressed Bytes %" PRIu64 "\n", total_benchmarks_uncompressed_size);

  unsigned int num_hist_sizes = 6;
  uint64_t hist_sizes[] = {
    64 << 10,
    32 << 10,
    16 << 10,
  ...
  unsigned int num_latency_injection_configs = 4;
  latency_info_t latency_injection_configs[] = {
    {1, false},
    {50, false},
    {400, false},
    {400, true}
  ...
  for (unsigned int m = 0; m < num_latency_injection_configs; m++) {
    ZstdCompressSetLatencyInjectionInfo((uint64_t) latency_injection_configs[m].cycles, (bool)latency_injection_configs[m].has_cache);
#ifdef DO_PRINT
    printf("Using Latency Cycles: %" PRIu64 " Cache Enabled: %d\n", latency_injection_configs[m].cycles, latency_injection_configs[m].has_cache);
#endif
      ...
      for (unsigned int j = 0; j < num_hist_sizes; j++) {
        ...
        for (unsigned int i = 0; i < num_benchmarks; i++) {
          if(run_benchmark((benchmark_compressed_data_arrays[i]), *(benchmark_compressed_data_len_array[i]),
                ...
                *(benchmark_names[i]), i, hist_sizes[j], result_area_decomp2, hash_table_sizes_log2[k], latency_injection_configs[m],
                &total_data_uncompressed_processed, &total_data_compressed_processed, &total_cycles_taken, &num_bench_passed, &benchmark_sum_overall,
                lit_buf,
                total_benchmarks_uncompressed_size,
                seq_buf,
                total_benchmarks_uncompressed_size,
        ...
        if (!is_warmup[j]) {
          printf("TOTAL: Took %" PRIu64 " cycles consumed %" PRIu64 " uncompressed bytes produced compsize %" PRIu64 " bytes SuccessNBenchmarks %d TotalNBenchmarks %d with histsram %" PRIu64 " with log2HTSize %" PRIu64 " latency %" PRIu64 " hasCache %d\n",
              total_cycles_taken, total_data_uncompressed_processed, total_data_compressed_processed, num_bench_passed, num_benchmarks, hist_sizes[j], hash_table_sizes_log2[k], latency_injection_configs[m].cycles, latency_injection_configs[m].has_cache);
        }
      }
    }
  }
