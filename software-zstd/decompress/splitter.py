...
for benchno, fileinfo in enumerate(my_chunk):
    ...
    subprocess.run(f"cp {input_file_raw} {uncompressed_varname}", shell=True)
    subprocess.run(f"cp {input_file_comp} {compressed_varname}", shell=True)
    subprocess.run(f"xxd -i {uncompressed_varname} >> {output_file}", shell=True)
    subprocess.run(f"xxd -i {compressed_varname} >> {output_file}", shell=True)
    subprocess.run(f"rm {compressed_varname}", shell=True)
    subprocess.run(f"rm {uncompressed_varname}", shell=True)
